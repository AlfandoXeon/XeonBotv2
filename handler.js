...
export default async (client, ctx) => {
   ...
   try {
      ...
      const isSpam = spam.detection(client, m, {
         ...
         show: 'none', // options: 'all' | 'command-only' | 'message-only' | 'spam-only'| 'none'
         banned_times: users?.ban_times,
         exception: isOwner || isPrem
      })

      // Aesthetic colorful log cards
      if (!m.fromMe) {
         const time = new Date().toLocaleTimeString('id-ID', { timeZone: 'Asia/Jakarta' })
         const sender = m.pushName || m.sender.split('@')[0]
         const room = m.isGroup ? groupMetadata.subject || 'Group' : 'Private'
         const isCmd = !!prefix && commands.includes(command)
         const logType = isCmd ? `COMMAND: ${prefix}${command}` : `MSG (${m.mtype || 'text'})`
         const content = body || (m.message ? Object.keys(m.message)[0] : '')
         ...
         const borderTitle = isCmd ? `${c.red}${c.bold}─[ COMMAND LOG ]` : `${c.green}${c.bold}─[ MESSAGE LOG ]`
         const borderLength = 100
         const titleLen = 16
         const borderTop = `${c.cyan}┌${borderTitle}${c.cyan}${'-'.repeat(borderLength - titleLen - 2)}┐${c.reset}`
         const borderBottom = `${c.cyan}└${'-'.repeat(borderLength - 2)}┘${c.reset}`
         
         const line1 = `${c.cyan}│${c.reset} ${c.yellow}Time:${c.reset} ${pad(time, 8)} ${c.cyan}│${c.reset} ${c.green}Sender:${c.reset} ${pad(sender, 25)} ${c.cyan}│${c.reset} ${c.magenta}Room:${c.reset} ${pad(room, 30)} ${c.cyan}│${c.reset}`
         const line2Content = isCmd ? `${c.red}${c.bold}${pad(`${logType} ➜ ${content}`, borderLength - 4)}${c.reset}` : `${c.white}${pad(`${logType} ➜ ${content}`, borderLength - 4)}${c.reset}`
         const line2 = `${c.cyan}│${c.reset} ${line2Content} ${c.cyan}│${c.reset}`
      ...
      if (
         ...
         for (const [pluginPath, pluginData] of Object.entries(is_commands)) {
            ...
            if (cmd.limit && !isPrem && users.limit < 1) {
            ...
            if (cmd.limit && !isPrem && users.limit > 0) {
         ...
         for (const [pluginPath, pluginData] of Object.entries(is_events)) {
            ...
            if (event.limit && !isPrem && !event.game && users.limit < 1 && body && Utils.generateLink(body) && Utils.generateLink(body).some(v => Utils.socmed(v))) return client.reply(m.chat, `⚠️ You reached the limit and will be reset at 00.00\n\nTo get more limits upgrade to premium plan.`, m).then(() => {
               users.premium = false
               users.expired = 0
            })
