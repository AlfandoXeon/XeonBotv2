...
function getPersona(config) {
   try {
      ...
      if (fs.existsSync(personaPath)) {
         let text = fs.readFileSync(personaPath, 'utf-8').trim()
...
WAKTU SEKARANG (Patokan Waktu Jakarta/WIB):
- Hari, Tanggal, Jam saat ini: ${wibTime} WIB
...
Jika user bertanya cara menggunakan fitur, cara download, atau daftar menu, beritahu mereka menggunakan perintah yang sesuai.`
   ...
   // Keep only the last 6 messages to avoid token bloat
   if (history.length > 6) {
